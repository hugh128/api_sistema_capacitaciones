...
const createHeader = (currentPage: number, pageCount: number, logo: string, fechaEmision: string): any => {
  return {
    margin: [25, 45, 25, 0],
    table: {
      widths: ['22%', '48%', '30%'],
...
const buildTrainingTypeRows = (data: CrearAsistenciaPdfDto): any[] => {
  const rows: any[] = [];

  // Checkbox Sistema documental
  rows.push([
    data.sistemaDocumental ? createCheckedBox() : createUncheckedBox(),
    { colSpan: 3, text: 'Sistema documental', fontSize: 11, margin: [-100, -4, 0, 0] },
    {},
    {}
  ]);
  ...
  // Checkbox TALLER
  rows.push([
    data.taller ? createCheckedBox() : createUncheckedBox(),
    { colSpan: 3, text: 'TALLER', fontSize: 11, margin: [-100, -4, 0, 0] },
    {},
    {}
  ]);
  ...
  // Checkbox CURSO
  rows.push([
    data.curso ? createCheckedBox() : createUncheckedBox(),
    { colSpan: 3, text: 'CURSO', fontSize: 11, margin: [-100, -4, 0, 0] },
    {},
    {}
  ]);
  ...
  // Checkbox OTRO
  rows.push([
    data.otro ? createCheckedBox() : createUncheckedBox(),
    { colSpan: 3, text: 'OTRO*', fontSize: 11, margin: [-100, -4, 0, 0] },
    {},
    {}
  ]);
...
const buildAttendeeRows = (asistentes: CrearAsistenciaPdfDto['asistentes']): any[] => {
...
export const crearListadoAsistencia = async (data: CrearAsistenciaPdfDto): Promise<Buffer> => {
  ...
  const docDefinition: TDocumentDefinitions = {
    pageSize: 'LETTER',
    pageMargins: [25, 135, 25, 60],
    
    header: (currentPage, pageCount) => {
      return createHeader(currentPage, pageCount, logo, fechaEmision);
    },
                ...
                table: {
                  ...
                  body: [
                    ...
                    [{
                      table: {
                        widths: ['40%', '25%', '20%', '15%'],
                        body: buildTrainingTypeRows(data)
                      },
                      layout: 'noBorders'
                    }]
                  ]
                    ...
                    [{
                      table: {
                        ...
                        body: [
                          [
                            {
                              ...(data.interno ? createCheckedBox() : createUncheckedBox()),
                              alignment: 'center',
                              margin: [0, 0, 0, 0]
                            } as any,
                          ...
                          [
                            {
                              ...(data.externo ? createCheckedBox() : createUncheckedBox()),
                              alignment: 'center',
                              margin: [0, -4, 0, 0]
                            } as any,
                            { text: 'Externo', fontSize: 11, margin: [-25, -4, 0, 0] }
                          ]
                        ]
                      },
                      layout: 'noBorders'
      ...
      // Tabla de asistentes
      {
        table: {
          ...
          body: [
            ...
            ...buildAttendeeRows(data.asistentes)
          ]
        },
        layout: {
          hLineWidth: () => 0.5,
